...
int sfs_write(int fd, int start, int length, byte *mem_pointer)
{
	...
	if(fd >= 0 && length > 0 && start >= -1)
	{
		...
		//TODO take the stuff below and turn it into a function so we can just call get_num_datablocks() on an index block and it'll give us the number of data blocks.
